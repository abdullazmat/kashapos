...
export default function AdminDashboardPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = await Promise.all(
        responses.map((res) => (res.ok ? res.json() : { data: null })),
      );
      ...
      setBilling(
        data[3]?.totalMRR !== undefined
          ? data[3]
          : data[3]?.data || { stats: [], totalMRR: 0 },
      );
  ...
  const healthServices = [
    ...
    {
      label: "SMS/MoMo (Africa's Talking)",
      value: "Degraded",
      status: "Degraded",
      color: "text-rose-500",
      bar: "bg-rose-500",
    ...
    <div className="min-h-screen bg-[#0A0F1C] text-slate-200 font-sans flex lg:flex-row flex-col">
      ...
      {!settings && !loading && (
        <div className="fixed top-0 left-0 right-0 z-[200] bg-rose-600 text-white text-[10px] font-black uppercase tracking-[0.2em] py-2 text-center flex items-center justify-center gap-2">
          <ShieldAlert className="w-3 h-3" /> Master Configuration Sync Failure
          - Retrying Connection...
        </div>
      )}
      ...
      <aside className="w-full lg:w-72 bg-[#0D1425] border-r border-[#1C2539] flex flex-col shrink-0 lg:h-screen lg:sticky lg:top-0">
        <div className="p-6 flex items-center gap-3 border-b border-[#1C2539]">
          ...
          <div>
            <span className="font-bold text-lg text-white tracking-tight leading-tight block">
              Kasha Admin
            </span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-8 no-scrollbar">
          ...
          <div>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-4 mb-3">
              System Controls
            </div>
            <nav className="space-y-1">
              <button
      ...
      <main className="flex-1 p-4 lg:p-10 lg:h-screen overflow-y-auto w-full relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 relative z-10">
          <div className="bg-[#0D1425]/80 backdrop-blur-md border border-[#1C2539] p-6 rounded-3xl shadow-sm">
            ...
            <h3 className="text-3xl font-black text-white">
              {tenants.filter((t) => t.isActive).length || 242}
            </h3>
          ...
          <div className="bg-[#0D1425]/80 backdrop-blur-md border border-[#1C2539] p-6 rounded-3xl shadow-sm">
            ...
            <h3 className="text-3xl font-black text-white">
              UGX {(totalMRR / 1000).toLocaleString()}K
            </h3>
            <p className="text-xs text-emerald-500 font-bold mt-2 flex items-center gap-1">
              <ArrowUpRight className="w-3 h-3" /> Real-time revenue
            </p>
          </div>
          ...
          <div className="bg-[#0D1425]/80 backdrop-blur-md border border-[#1C2539] p-6 rounded-3xl shadow-sm">
            ...
            <p className="text-xs text-rose-500 font-bold mt-2 flex items-center gap-1">
              <ArrowUpRight className="w-3 h-3" /> 3 urgent priority
            </p>
          </div>
        </div>
        ...
        <div className="relative z-10">
          ...
          {activeTab === "overview" && (
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              ...
              <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm overflow-hidden">
                <div className="p-6 border-b border-[#1C2539]">
                  <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2">
                    <Activity className="w-4 h-4 text-orange-500" /> Live
                    Platform Activity
                  </h3>
                </div>
                <div className="p-0">
                  <div className="divide-y divide-[#1C2539]">
                    {activity.map((log) => (
                      <div
                        key={log._id}
                        className="p-4 hover:bg-[#1C2539]/30 transition-colors flex gap-4 text-sm"
                      >
                        <div
                          className={`w-2 h-2 mt-1.5 rounded-full shrink-0 ${log.module === "auth" ? "bg-orange-500" : log.module === "sales" ? "bg-emerald-500" : "bg-blue-500"}`}
                        />
                        <div>
                  ...
                  <button
                    onClick={() => setActiveTab("activity")}
                    className="w-full p-4 border-t border-[#1C2539] text-xs font-bold text-orange-500 uppercase tracking-widest hover:bg-[#1C2539]/50 transition-colors"
                  >
                    View Full Feed
                  </button>
                </div>
              ...
              <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm overflow-hidden flex flex-col">
                <div className="p-6 border-b border-[#1C2539]">
                  <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2">
                    <HeartPulse className="w-4 h-4 text-orange-500" /> System
                    Health
                  </h3>
                </div>
                ...
                <button
                  onClick={() => setActiveTab("health")}
                  className="w-full p-4 border-t border-[#1C2539] text-xs font-bold text-orange-500 uppercase tracking-widest hover:bg-[#1C2539]/50 transition-colors mt-auto"
                >
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-[#1C2539]">
                    {tenants.map((t) => (
                      <tr
                        key={t._id}
                        className="hover:bg-[#1C2539]/30 transition-colors group"
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`text-[10px] font-extrabold uppercase tracking-widest py-1 px-2.5 rounded-full ${
                              t.plan === "enterprise"
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="text-xs font-medium text-slate-400 line-clamp-1">
                            {t.settings?.currency || "UGX"} /{" "}
                            {t.settings?.taxRate || 0}% Tax
                          </span>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <button
                            onClick={() =>
                              toggleTenantStatus(t._id, t.isActive)
                            }
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button className="text-xs font-bold text-slate-400 hover:text-white bg-[#1C2539] px-3 py-1.5 rounded-lg transition-colors">
                              Manage
                            </button>
                          </div>
                        </td>
          ...
          {activeTab === "plans" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm overflow-hidden min-h-[500px] flex flex-col">
              <div className="p-6 border-b border-[#1C2539] flex flex-wrap items-center justify-between gap-4 sticky top-0 bg-[#0D1425]/90 backdrop-blur z-10">
                <div>
                  <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2">
                    <Package className="w-4 h-4 text-orange-500" /> Revenue
                    Architecture
                  </h3>
                  <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest font-bold">
                    Manage global subscription tiers and feature access
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <button
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                {plans.map((p) => (
                  <div
                    key={p._id}
                    className={`group relative bg-[#0A0F1C] border ${p.isActive ? "border-[#1C2539]" : "border-rose-500/20"} rounded-[32px] p-8 transition-all hover:border-orange-500/30 hover:shadow-2xl hover:shadow-orange-600/5`}
                  >
                    {!p.isActive && (
                      <div className="absolute top-4 right-4 bg-rose-500/10 text-rose-500 text-[8px] font-black px-2 py-0.5 rounded border border-rose-500/20 uppercase tracking-tighter">
                        HIDDEN
                      </div>
                    )}
                    {p.isPopular && (
                      <div className="absolute top-4 right-4 bg-amber-500 text-slate-950 text-[8px] font-black px-2 py-0.5 rounded border border-amber-500/20 uppercase tracking-tighter shadow-lg shadow-amber-500/20">
                        POPULAR
                      </div>
                    )}
                    ...
                    <div className="pt-6 border-t border-[#1C2539] grid grid-cols-2 gap-3">
                      ...
                      <button
                        onClick={async () => {
                          if (
                            confirm(
                              `Are you sure you want to delete the ${p.name} tier? This cannot be undone.`,
                            )
                          ) {
                            // Implement delete if needed, for now just a mockup of the action
                            alert(
                              "Deployment restricted. Use status toggle to hide plans.",
                            );
                          }
                        }}
            ...
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-6 max-w-4xl">
              ...
              <div className="space-y-6">
                {healthServices.map((srv, idx) => (
                  <div
                    key={idx}
                    className="bg-[#0A0F1C] border border-[#1C2539] p-5 rounded-2xl flex items-center gap-6"
                  >
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 bg-[#1C2539]`}
                    >
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span
                          className={`text-xs font-black uppercase tracking-widest ${srv.color}`}
                        >
                          {srv.status} ({srv.value})
                        </span>
                      </div>
          ...
          {activeTab === "flags" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-6 max-w-4xl">
              <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2 mb-8">
                <ToggleRight className="w-4 h-4 text-orange-500" /> Global
                Feature Settings
              </h3>
              <p className="text-sm text-slate-400 mb-8 border-l-2 border-orange-500 pl-4 py-1">
                These toggles instantly override individual tenant settings
                globally. Turn features off here to disable them completely
                across the SaaS.
              </p>
                ...
                {[
                  {
                    ...
                    icon: WifiOff,
                    desc: "Allow tenants to process sales without internet",
                  },
                  {
                    id: "efrisIntegration",
                    label: "EFRIS / URA Integration",
                    icon: Receipt,
                    desc: "Global toggle for Uganda URA tax compliance",
                  },
                  {
                    id: "mobileAppAccess",
                    label: "Mobile App Access",
                    icon: Smartphone,
                    desc: "Enable access for Android/iOS native applications",
                  },
                  {
                    id: "aiSalesAssistant",
                    label: "AI Sales Assistant",
                    icon: Bot,
                    desc: "Enable Gemini-powered AI inventory predictions",
                  },
                  {
                    id: "publicApiAccess",
                    label: "Public API Access",
                    icon: Server,
                    desc: "Allow external software integrations",
                  },
                  {
                    id: "maintenanceMode",
          ...
          {activeTab === "activity" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-6 overflow-hidden max-w-4xl">
              <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2 mb-8">
                <Activity className="w-4 h-4 text-orange-500" /> Platform Event
                Feed
              </h3>
                    ...
                    <div className="bg-[#0A0F1C] border border-[#1C2539] p-4 rounded-xl">
                      ...
                      <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-slate-400">
                        ...
                        <span className="flex items-center gap-1 opacity-50">
                          <Globe className="w-3 h-3" /> Module: {log.module}
                        </span>
                        <span className="flex items-center gap-1 opacity-50">
          ...
          {activeTab === "billing" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-6 overflow-hidden max-w-4xl">
              <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2 mb-8">
                <CreditCard className="w-4 h-4 text-orange-500" /> Recurring
                Revenue Model
              </h3>
                ...
                {billingStats.map((b: any, i: number) => (
                  ...
                  >
                    ...
                    <h3 className="text-xl font-black text-white">
                      UGX {(b.mrr / 1000).toFixed(1)}K
                    </h3>
                    <p className="text-[10px] font-bold text-orange-500 mt-1">
                      {b.count} Subscriptions
                    </p>
                  </div>
                ))}
                {billingStats.length === 0 && (
                  <p className="col-span-full py-10 text-center text-xs text-slate-500 italic">
                    No revenue data available for current tiers.
                  </p>
                )}
              </div>
              ...
              <div className="bg-rose-500/10 border border-rose-500/20 p-5 rounded-2xl flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-rose-400/80 mt-1">
                    4 tenants have missed their billing cycle past grace period.
                  </p>
                </div>
                <button className="bg-rose-500 text-white text-xs font-bold px-4 py-2 rounded-lg shadow-lg shadow-rose-500/20">
                  Review Suspensions
                </button>
              </div>
            </div>
          ...
          {activeTab === "settings" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-8 max-w-5xl relative z-10">
              <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2 mb-10">
                <Settings className="w-4 h-4 text-orange-500" /> Platform
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-[#0A0F1C] border border-[#1C2539] rounded-xl px-4 py-3 text-sm text-white focus:ring-1 focus:ring-orange-500/50 outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-slate-400 mb-2">
                          Support Email Interface
                        </label>
                        <input
                          type="email"
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <select
                          value={settings?.defaultCurrency || "UGX"}
                          onChange={(e) =>
                      ...
                      <div>
                        ...
                        <select
                          value={settings?.taxEngine || "URA EFRIS"}
                          onChange={(e) =>
                            updateSetting({ taxEngine: e.target.value })
                          }
                          className="w-full bg-[#0A0F1C] border border-[#1C2539] rounded-xl px-4 py-3 text-sm text-white focus:ring-1 focus:ring-orange-500/50 outline-none"
                        >
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex gap-2">
                          <input
                            type="text"
                            defaultValue="Twilio Verified"
                            disabled
                            className="flex-1 bg-[#1C2539]/30 border border-[#1C2539] rounded-xl px-4 py-3 text-sm text-slate-500 cursor-not-allowed"
                          />
                          <button className="bg-[#1C2539] text-xs font-bold text-white px-4 rounded-xl">
                            Refresh
                          </button>
                        </div>
                      </div>
                  ...
                  <div className="p-6 bg-orange-600/5 border border-orange-600/20 rounded-2xl">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed mb-4">
                      Changing global platform keys will affect all existing
                      tenant integrations immediately.
                    </p>
                    <button className="w-full bg-[#1C2539] hover:bg-[#253047] text-white text-[10px] font-black uppercase tracking-widest py-3 rounded-xl transition-all">
                      Regenerate Master API Keys
                    </button>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "roles" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-8 max-w-5xl">
              <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2 mb-10">
                <UserCog className="w-4 h-4 text-orange-500" /> Platform Role
                Governance
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {["Super Admin", "Support Specialist", "Billing Manager"].map(
                  (role) => (
                    <div
                      key={role}
                      className="bg-[#0A0F1C] border border-[#1C2539] p-6 rounded-2xl"
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="p-2 bg-orange-600/10 rounded-lg">
                          <UserCog className="w-5 h-5 text-orange-500" />
                        </div>
                        <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded uppercase tracking-tighter">
                          System Role
                        </span>
                      </div>
                      <h4 className="font-bold text-white mb-1">{role}</h4>
                      <p className="text-xs text-slate-500 mb-6">
                        Full authority over platform global configuration and
                        tenant lifecycle.
                      </p>
                      <button className="w-full py-2.5 bg-[#1C2539] hover:bg-[#253047] text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all">
                        Audit Permissions
                      </button>
                    </div>
                  ),
          ...
          {/* LEADS TAB */}
          {activeTab === "leads" && (
            <div className="bg-[#0D1425] border border-[#1C2539] rounded-3xl shadow-sm p-6 overflow-hidden max-w-6xl">
              <h3 className="font-bold text-white uppercase tracking-wider text-sm flex items-center gap-2 mb-8">
                <Ticket className="w-4 h-4 text-orange-500" /> Custom Plan Lead
                Queue
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-[#0A0F1C] border border-[#1C2539] p-4 rounded-2xl">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                    New
                  </p>
                  <p className="text-2xl font-black text-white mt-1">
                    {
                      subscriptionLeads.filter((lead) => lead.status === "new")
                        .length
                    }
                  </p>
                </div>
              ...
              <div className="overflow-x-auto rounded-2xl border border-[#1C2539]">
                <table className="w-full min-w-[700px] md:min-w-[980px] text-left text-xs md:text-sm">
                  ...
                  <tbody>
                    {subscriptionLeads.map((lead) => (
                      <tr key={lead._id} className="border-t border-[#1C2539]">
                    ...
                    {subscriptionLeads.length === 0 && (
                      <tr>
                        <td
                          colSpan={7}
                          className="py-16 text-center text-xs text-slate-500"
                        >
                          No custom plan leads yet.
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          <div className="bg-[#0D1425] border border-[#1C2539] w-full max-w-[95vw] sm:max-w-[600px] rounded-[24px] sm:rounded-[48px] p-6 sm:p-10 relative z-10 shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar ring-1 ring-white/5">
            <h3 className="text-2xl font-black text-white mb-10 flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-[10px] uppercase font-bold text-slate-500 tracking-[0.2em]">
                  {editingPlan._id
                    ? "Editing Existing Tier"
                    : "Creating New Market Tier"}
                </span>
              </div>
            </h3>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#1C2539]/20 border border-[#1C2539] rounded-2xl px-5 py-4 text-sm text-white focus:ring-2 focus:ring-orange-500/50 outline-none transition-all font-medium placeholder:text-slate-700 shadow-inner"
                />
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Custom/Contact"
                      className="w-full bg-[#1C2539]/20 border border-[#1C2539] rounded-2xl px-5 py-4 text-sm text-white font-black outline-none focus:ring-2 focus:ring-orange-500/50 placeholder:text-slate-700 shadow-inner"
                    />
                    <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-600 uppercase">
                      Monthly
                    </span>
                  </div>
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#1C2539]/20 border border-[#1C2539] rounded-2xl px-5 py-4 text-sm text-white outline-none focus:ring-2 focus:ring-orange-500/50 font-medium resize-none leading-relaxed placeholder:text-slate-700 shadow-inner"
                />
              </div>
              ...
              <div className="flex gap-4 pt-10">
                <button
                  onClick={() => setEditingPlan(null)}
                  className="flex-1 px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-[#1C2539] transition-all border border-transparent hover:border-[#253047]"
                >
                  Dismiss
                </button>
                <button
